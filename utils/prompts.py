...
DOCSTRING_WRITER = (
    ...
    "as if it already existed. Make it is easy to infer from the description which use cases and "
    "contexts the function applies to.\n"
    "- In the 'Example' section: Call the function like so: `>>> function_name(arg, [...], kwarg=val, "
    "[...])`. Make sure to include the names of all and only of the keyword arguments. Provide only one "
    "example with arguments for a representative use case. Do not show the return value of the function call.\n"
...
CODER = (
    ...
    "diverse use cases and contexts. The function must be type hinted, working, and every aspect must be implemented "
    ...
    "Do not use placeholders that must be filled in manually (e.g. API keys).\n"
